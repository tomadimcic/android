...
import dunavnet.com.mojns.tasks.LoginSiginTask;
...
public class LoginActivity extends Activity implements ResponseHandler {
    ...
    public void login() {
        sendLogData();
        Intent i = new Intent(LoginActivity.this, MainActivityList.class);
        startActivity(i);
        finish();
    }
        ...
        if(mCurrentRequest == HTTP_GET_REQUEST){
            ...
            try {
                ...
                if (response.contains("organizations")) {
                    Toast.makeText(LoginActivity.this, "Welcome: " + json.getString("displayName") + "\r\nYour access credentials are valid.", Toast.LENGTH_SHORT).show();
                    prefs.edit().putString("email", json.getString("email")).commit();
                    login();
                }
                else {
                    if(flag == 0) {
                        Toast.makeText(LoginActivity.this, "Your access credentials are ider expired or not valid. Try again to login!", Toast.LENGTH_SHORT)
                                .show();
                        openLoginForm();
                    }
